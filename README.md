...
国内深潜这次按公开手册对过几何表，空项继续空着：

- **hecoos** Quick Guide 写明显示面含穹顶和带 UV 的 OBJ，软件不能改 UV。逐步例子是投影标定。实时视锥没有步骤，保持空。
- **GrandMapping** 产品页写了导入模型、相机视角、按行列或分辨率切片、导入卡莱特发送端连接关系。模型格式和文件字段没写，格子仍是部分。AI 推断和「球形」宣传不升格。
- **Kommander** 的异形章（T1 §7.6、2025 手册 §7.8）是矩形拼进显示口再旋转。360 和穹顶是素材特效名，不是屏体 UV 表。
- **HiRender S3** 的网格是投影几何变形。OBJ 和屏体 UV 未检索到。
- **Kompass** 的走线在 SmartLCT / 接收卡。FX 手册目录打开了，正文里没有 OBJ/UV 章节，播控几何保持空。
...
3. **同步组建**：建 Framelock Group，输出黑白格。测试图烧上 Sync 卡帧计数（没有卡就用 VBlank 计数）、节点名和输出口。相邻箱体上的帧号相同，格线不错位。
4. **工程导入**：模型、切片、素材路径、时间线。预监和输出一致。
5. **跳转测试**：每个 Cue 跳一次。240 fps 拍接缝时，两侧烧录帧号必须相同。日志记 `targetFrame`、已就绪节点 `ready`、未齐时保持的旧帧 `held`，以及 `COMMITTED` 或 `ABORTED`。声画用同一条 LTC：视频侧对 present 帧号，音频侧对声卡回路上的节目声起振点。只在观众位听，不能代替这条记录。
